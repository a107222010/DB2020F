...
namespace A107222008_UsedCar
{
    public partial class frmBCarTransaction : Form
    {
        ...
        {
            sqlStr = $"SELECT orderform.OrderID AS '訂單編號',salescar.Brand AS '廠牌',salescar.CarType AS '車款',salescar.CarStyle AS '車種',salescar.CarAge AS '車齡', orderform.OrderDate AS '訂單日期', orderform.LicensePlate AS '車牌', orderform.CarDealerID AS '賣家編號' FROM orderform LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate WHERE orderform.BuyerID = '{BuyerID}' AND orderform.OrderID NOT IN(SELECT record.OrderID FROM record)";
        ...
        {
            sqlStr = $"SELECT record.RecordID AS '紀錄編號',salescar.Brand AS '廠牌',salescar.CarType AS '車款',salescar.CarStyle AS '車種',salescar.CarAge AS '車齡', record.RecordDate AS '紀錄日期', orderform.CarDealerID AS '賣家編號', record.CDCreditRating AS '賣家評價'  FROM((record LEFT JOIN orderform ON record.OrderID = orderform.OrderID) LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate) WHERE orderform.BuyerID = '{BuyerID}'";
            mySqlDataAdapter = new MySqlDataAdapter(sqlStr, conn);
            ds = new DataSet();
            mySqlDataAdapter.Fill(ds);
        ...
        private void updateCbx()
        {
            ...
            sqlStr = $"SELECT record.RecordID FROM record LEFT JOIN orderform ON record.OrderID = orderform.OrderID WHERE orderform.BuyerID = '{BuyerID}'";
            ...
            sqlStr = $"SELECT orderform.OrderID FROM orderform WHERE orderform.BuyerID = '{BuyerID}' AND orderform.OrderID NOT IN (SELECT record.OrderID FROM record)";
            cmd = new MySqlCommand(sqlStr, conn);
        ...
        private void btnGiveCR_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (tbxCR.Text != "")
                {
                    ...
                    {
                        sqlStr = $"UPDATE record SET CDCreditRating = {tbxCR.Text} WHERE record.RecordID = '{cbxRI.Text}'";
                        cmd = new MySqlCommand(sqlStr, conn);
                        cmd.ExecuteNonQuery();
