...
namespace A107222008_UsedCar
{
    public partial class frmSCreditRating : Form
    {
        ...
        private void frmSCreditRating_Load(object sender, EventArgs e)
        {
            ...
            updateCbx();
            int CRaverage = 0, i = 0;
            sqlStr = $"SELECT record.CDCreditRating FROM record LEFT JOIN orderform ON record.OrderID = orderform.OrderID WHERE orderform.CarDealerID = '{SellerID}'";
            ...
            sqlStr = $"SELECT record.RecordID AS '完成編號', salescar.Brand AS '廠牌',salescar.CarType AS '車型',salescar.CarStyle AS '車種',salescar.CarAge AS '車齡',record.RecordDate AS '完成時間', orderform.CarDealerID AS '賣家編號', record.CDCreditRating AS '賣家評價' FROM((record LEFT JOIN orderform ON record.OrderID = orderform.OrderID) LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate) WHERE orderform.CarDealerID = '{SellerID}'";
        ...
        private void btnCreditRating_Click(object sender, EventArgs e)
        {
            ...
            int CRaverage = 0, i = 0;
            sqlStr = $"SELECT record.BuyerCreditRating FROM record LEFT JOIN orderform ON record.OrderID = orderform.OrderID WHERE orderform.BuyerID = '{cbxbuyerID.Text}'";
            cmd = new MySqlCommand(sqlStr, conn);
            ...
            sqlStr = $"SELECT orderform.CarDealerID AS '買家編號',salescar.Brand AS '廠牌' ,salescar.CarType AS '車型' , salescar.CarStyle AS '車種' ,salescar.CarAge AS '車齡',record.RecordDate AS '完成紀錄', record.CDCreditRating AS '買家評價'  FROM((record LEFT JOIN orderform ON record.OrderID = orderform.OrderID) LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate) WHERE orderform.BuyerID = '{cbxbuyerID.Text}'";
            cmd = new MySqlCommand(sqlStr, conn);
            mySqlDataAdapter = new MySqlDataAdapter(sqlStr, conn);
            ds = new DataSet();
            mySqlDataAdapter.Fill(ds);
