...
namespace A107222008_UsedCar
{
    public partial class frmSCarTransaction : Form
    {
        ...
        private void showOneTable()
        {
            ...
            sqlStr = $" SELECT orderform.OrderID AS '訂單編號',salescar.Brand AS '廠牌',salescar.CarType AS '車款',salescar.CarStyle AS '車種',salescar.CarAge AS '車齡' , orderform.OrderDate AS '訂單日期', orderform.LicensePlate AS '車牌', orderform.CarDealerID AS '賣家編號' FROM orderform LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate WHERE orderform.CarDealerID = '47826' AND orderform.OrderID NOT IN(SELECT record.OrderID FROM record)";
        ...
        private void showTwoTable()
        {
            ...
            sqlStr = $"SELECT record.RecordID AS '紀錄編號',salescar.Brand AS '廠牌',salescar.CarType AS '車款',salescar.CarStyle AS '車種',salescar.CarAge AS '車齡', record.RecordDate AS '紀錄日期', orderform.BuyerID AS '買家編號', record.BuyerCreditRating AS '買家評價' FROM((record LEFT JOIN orderform ON record.OrderID = orderform.OrderID) LEFT JOIN salescar ON orderform.LicensePlate = salescar.LicensePlate) WHERE orderform.CarDealerID = '{SellerID}'";
            mySqlDataAdapter = new MySqlDataAdapter(sqlStr, conn);
            ds = new DataSet();
            mySqlDataAdapter.Fill(ds);
        ...
        public void updateCbx()
        {
            ...
            sqlStr = $"SELECT record.RecordID FROM record LEFT JOIN orderform ON record.OrderID = orderform.OrderID WHERE orderform.CarDealerID = '{SellerID}'";
            ...
            sqlStr = $"SELECT orderform.OrderID FROM orderform WHERE orderform.CarDealerID = '{SellerID}' AND orderform.OrderID NOT IN (SELECT record.OrderID FROM record)";
            cmd = new MySqlCommand(sqlStr, conn);
        ...
        {
            sqlStr = $"SELECT RecordDate FROM record WHERE RecordID = '{cbxRI.Text}'";
            cmd = new MySqlCommand(sqlStr, conn);
            ...
            else
            {
                ...
                try
                {
                    if (tbxCR.Text != "")
                    {
                        ...
                        {
                            sqlStr = $"UPDATE record SET BuyerCreditRating = {tbxCR.Text} WHERE record.RecordID = '{cbxRI.Text}'";
                            cmd = new MySqlCommand(sqlStr, conn);
                            cmd.ExecuteNonQuery();
